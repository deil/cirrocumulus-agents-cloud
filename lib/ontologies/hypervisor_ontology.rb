  ...
  def collect_initial_guest_stats
    ...
    running_guests.each do |guest_id|
      ...
      guest.interfaces.each_with_index do |vif, idx|
        debug "-> vif#{idx} rx=%.02f tx=%.02f" % [vif[:rx]/(1024*1024), vif[:tx]/(1024*1024)]
        assert [:guest, guest_id, :vif, idx, :rx, vif[:rx], :tx, vif[:tx]]
      end

      guest.block.each_key do |dev|
        debug "-> #{dev} reads=%d (%.02f Mb) writes=%d (%.02f Mb)" % [guest.block[dev][:rd_req], guest.block[dev][:rd_bytes]/(1024*1024), guest.block[dev][:wr_req], guest.block[dev][:wr_bytes]/(1024*1024)]
      end
    end
  end
